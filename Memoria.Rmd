...
```{r figura_triple2, echo=FALSE, fig.cap="Histogramas de las variables píxeles oscuros, rango de V y media de V con filtro blur", fig.width=15, fig.height=4, out.width="100%", fig.align="center"}
par(mfrow = c(1, 3), mar = c(0, 0.5, 2, 0.5)) 
...
El primer modelo multiclase se construyó utilizando las mejores variables obtenidas en la clasificación binaria: 
`R_mediana`, `G_mediana`, `B_mediana`, `H_cos_mediana`, `H_sin_mediana`, `S_mediana`, `V_mediana` y `sd_V`. Realizando un entrenamiento con estas variables pero con los datos de las tres clases, este modelo alcanza un accuracy del 76.7% en validación. En general, el modelo presentaba una buena separación entre Día y Noche, pero una mayor confusión con la clase Crepúsculo, lo que refleja su naturaleza intermedia. Las variables más importantes según el índice MeanDecreaseGini son `G_mediana` y `V_mediana`, confirmando que la luminosidad y el color dominante del cielo son los factores principales para diferenciar los distintos momentos del día.

#### Adición de la proporción de tonos cálidos
...
```{r figura_c2, echo=FALSE, fig.cap="Diagrama de cajas (izquierda) e histogramas (derecha) de la desviación estándar de V", out.width="80%", fig.align="center", fig.height=2.75}
library(png)
...
```{r figura15, echo=FALSE, fig.cap="Imagen mal clasificada para el modelo binario Día/Noche", out.width="70%", fig.align="center", fig.height=2.75}
...
El modelo seleccionado para la distinción entre cielosnublados y despejados eclasifica nuestras imágenes de test con una exactitud balanceada del 81%. Fijándonos en la matriz de confusión comprobamos que solo clasifica erróneamente una imagen del cielo despejado, mientras que clasifica 18 imágenes de cielos nublados incorrectamente. La mayoría de las imágenes de cielos nublados mal clasificadas o bien son recortes muy homogéneos, o bien son trozos de cielo azul (recorte no adecuado), habiendo solo una imagen de nubes bien definidas que haya sido mal clasificada. Por otra parte la única imagen de cielo despejado mal clasificada presenta un cielo grisáceo y un tercio de la imagen es una arboleda, cuya presencia puede haber confundido al clasificador. Dicha imagen y otras tres mal clasificadas de la clase Nublado pueden encontrarse en la Figura \ref{fig:figuraDN}.

```{r figuraDN, echo=FALSE, fig.cap="Imágenes mal clasificadas para el modelo meteorológico", out.width="80%", fig.align="center", fig.height=2.75}
