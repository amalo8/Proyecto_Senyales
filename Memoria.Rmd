...
header-includes:
  ...
  - \usepackage{float}
  - \usepackage{etoolbox}
  - \patchcmd{\thebibliography}{\itemsep\z@}{\itemsep=1\baselineskip}{}{}
---
...
Todos los materiales utilizados en el proceso de desarrollo de este proyecto se encuentran en el repositorio público de Github https://github.com/amalo8/Proyecto_Senyales.git.
...
knitr::include_graphics("figuras/figura1.png")
```
...
knitr::include_graphics("figuras/figura2.png")
```
...
img1 <- readPNG("figuras/boxplotRGB.png")
plot.new() 
rasterImage(img1, 0, 0, 1, 1)

img2 <- readPNG("figuras/histogramasRGB.png")
plot.new()
...
```{r figura_triple, echo=FALSE, fig.cap="Histogramas de las variables Sobel, fuentes de luz y entropía", fig.width=15, fig.height=4, out.width="100%", fig.align="center"}
par(mfrow = c(1, 3), mar = c(0, 0.5, 2, 0.5)) 

img1 <- readPNG("figuras/histogramaSOBEL.png")
plot(1:10, type="n", axes=FALSE, xlab="", ylab="", main="Característica detección de bordes (Sobel)")
rasterImage(img1, 1, 1, 10, 10)

img2 <- readPNG("figuras/histogramaLUZ.png")
plot(1:10, type="n", axes=FALSE, xlab="", ylab="", main="Característica detección fuentes de luz")
rasterImage(img2, 1, 1, 10, 10)

img3 <- readPNG("figuras/histogramaENTROPIA.png") 
...
```{r figura_triple2, echo=FALSE, fig.cap="Histogramas de las variables píxeles oscuros, rango de V y media de V con filtro blur", fig.width=15, fig.height=4, out.width="100%", fig.align="center", fig.pos="H"}
par(mfrow = c(1, 3), mar = c(0, 0.5, 2, 0.5)) 

img1 <- readPNG("figuras/histogramaPIXEL_OSC.png")
plot(1:10, type="n", axes=FALSE, xlab="", ylab="", main="Característica porcentaje de píxeles oscuros")
rasterImage(img1, 1, 1, 10, 10)

img2 <- readPNG("figuras/histogramaRANGO_V.png")
plot(1:10, type="n", axes=FALSE, xlab="", ylab="", main="Característica rango de V")
rasterImage(img2, 1, 1, 10, 10)

img3 <- readPNG("figuras/histogramaBLUR.png") 
...
```{r figura6, echo=FALSE, fig.cap="Diagrama de cajas (izquierda) e histogramas (derecha) de la desviación estándar de V", out.width="70%", fig.align="center", fig.width=12, fig.height=4}
library(png)
par(mfrow = c(1, 2), mar = c(0, 0, 0, 0)) 
img1 <- readPNG("figuras/boxplotSDV.png")
plot.new() 
rasterImage(img1, 0, 0, 1, 1)

img2 <- readPNG("figuras/histogramaSDV.png")
plot.new()
rasterImage(img2, 0, 0, 1, 1)
...
### Nuevas Características para detectar la clase Crepúsculo
...
#### Incorporación de gradientes por bandas horizontales
...
knitr::include_graphics("figuras/RGB_despejado_nublado.png")
```
...
knitr::include_graphics("figuras/HSV_despejado_nublado.png")
```
...
knitr::include_graphics("figuras/bcr_despejado_nublado.png")
```
...
```{r figura10, echo=FALSE, fig.cap="Histogramas de la saturación (S) media por bloque", out.width="100%", fig.align="center", fig.pos="H"}

knitr::include_graphics("figuras/bloques_despejado_nublado.png")
```
...
```{r figura11, echo=FALSE, fig.cap="Mapa de correlaciones entre las variables para la clasificación meteorológica", out.width="80%", fig.align="center", fig.height=2.75, fig.pos="H"}

knitr::include_graphics("figuras/corrplot.png")
```

```{r figura12, echo=FALSE, fig.cap="Proporciones y proporciones acumuladas de varianza explicada por el PCA", out.width="80%", fig.align="center", fig.height=2.75, fig.pos="H"}

knitr::include_graphics("figuras/pca_varianzas.png")
```

Después de entrenar nuestro modelo con las 15 primeras componentes resultado de la PCA, en la etapa de validación el modelo ha alcanzado un accuracy del 95,62%. Además, al analizar la importancia de las características mediante el índice \textit{MeanDecreaseGini}, se observa que las componentes principales influyen más significativamente en orden descendente (es decir, primero `PC1`, después `PC2`, y así sucesivamente), por su propia naturaleza.
...
A partir de nuestros resultados podemos comprobar que la nueva clase, Crepúsculo, es la que ha dado lugar a mayor confusión en el modelo: del las 14 imágenes mal clasificadas 13 de los errores vienen dados por imágenes de la categoría Crepúsculo siendo clasificadas como Día/Noche o imágenes de las categorías Día/Noche siendo clasificadas como Crepúsculo. La tendencia a confundir la categoría Crepúsculo queda también reflejada en sus correspondientes estadísticos de la matriz de confusión, que son menores con respecto a los de las otras clases.  
...
```{r figura13, echo=FALSE, fig.cap="Imágenes mal clasificadas para el modelo multiclase Día/Noche/Crepúsculo", out.width="80%", fig.align="center", fig.height=2.75}

knitr::include_graphics("figuras/mal_multiclase_plano.png")
```
  
Realizando una inspección visual de las imágenes mal clasificadas, como se ejemplifica en la Figura \ref{fig:figura13}, encontramos que las imágenes de Noche mal clasificadas como Crepúsculo presentan buena iluminación (para tratarse de una imagen nocturna), fuentes de luz claras y cierta presencia de tonos rojizos, similarmente las única imagen de Noche mal clasificada como Día presenta también buena iluminación y tonos muy vivos. En cuanto a Crepúsculo la mayoría de las imágenes mal clasificadas como Día presentan una importante proporción de tonos azules en el cielo, mientras que la única imagen predicha como Noche presenta tonos muy oscuros en gran parte de la imagen. Finalmente, para la categoría Día no se encuentra ninguna imagen erróneamente etiquetada como Noche, pero sí algunas para las que se ha predicho la categoría Crepúsculo, en nuestro conjunto de test las imágenes para las que se ha encontrado este error presentan una parte sombreada en la parte inferior y horizontes claramente definidos, que quizá puede resultar similar al paisaje característico de una imagen durante el crepúsculo.  


## Resultados en Clasificación Meteorológica

Hemos evaluado el modelo del apartado 5.2 sobre un total de 100 imágenes de test, siendo 50 de la clase nublado y 50 de las clase Despejado. Realizando las predicciones y comparando con las etiquetas reales obtenemos las siguientes métricas de evaluación:

| Exactitud | Sensibilidad | Especificidad | Precisión |   F1  |
|:---------:|:------------:|:-----------:|:---------:|:-----:|
|   0.810   |     0.640    |      0.980  |   0.969   | 0.771 |
Table: Estadísticos de la matriz de confusión para el test del clasificador meteorológico binario. 

El modelo seleccionado para la distinción entre cielos nublados y despejados clasifica nuestras imágenes de test con una exactitud balanceada del 81%. Fijándonos en la matriz de confusión comprobamos que solo clasifica erróneamente una imagen del cielo despejado, mientras que clasifica 18 imágenes de cielos nublados incorrectamente. La mayoría de las imágenes de cielos nublados mal clasificadas o bien son recortes muy homogéneos, o bien son trozos de cielo azul (recorte no adecuado), habiendo solo una imagen de nubes bien definidas que haya sido mal clasificada. Por otra parte la única imagen de cielo despejado mal clasificada presenta un cielo grisáceo y un tercio de la imagen es una arboleda, cuya presencia puede haber confundido al clasificador. Dicha imagen y otras tres mal clasificadas de la clase Nublado pueden encontrarse en la Figura \ref{fig:figuraDN}.

```{r figuraDN, echo=FALSE, fig.cap="Imágenes mal clasificadas para el modelo meteorológico", out.width="80%", fig.align="center", fig.height=2.75,fig.pos="H"}

knitr::include_graphics("figuras/mal_clasificadas_despejado_nublado.png")
```


## Conclusiones y Líneas Futuras  

```{r tabla_resumen, echo=FALSE, results='asis', message=FALSE, warning=FALSE}

library(kableExtra)

datos_tabla <- data.frame(
  Clasificador = c("Temporal binario", "Temporal multiclase", "Meteorológico"),
  Variables = c(
    "Mediana RGB y HSV (R, G, B, H_cos, H_sin, S, V); desviación estándar de V.",
    "Mediana RGB/HSV + sd(V) + % Tonos Cálidos + % Píxeles Extremos + Estadísticas por bloques espaciales (3x1).",
    "Mediana RGB/HSV + Ratios (Azul/Blancos/Grises) + Brillo/Contraste + Sobel + Saturación por bloques → PCA (15 componentes)."
  ),
  `Exactitud Validación` = c("90.15 %", "81.20 %", "95.62 %"),
  `Exactitud Test` = c("98.80 %", "88.30 %", "81.00 %")
)
...
knitr::kable(
  ...
  format = "latex",
  booktabs = TRUE,
  caption = "Resumen de características y métricas de rendimiento de los modelos finales",
  align = "llcc"   # ← SOLO l c r
) %>%
  kable_styling(
...
El desarrollo de este proyecto ha permitido demostrar que la extracción manual de características, combinada con algoritmos de aprendizaje clásico como Random Forest, es una estrategia viable y efectiva para la clasificación de imágenes del cielo, ofreciendo una alternativa interpretable a los modelos de caja negra del aprendizaje profundo.

En cuanto a la clasificación temporal, se ha demostrado que la distinción binaria entre día y noche es un problema robusto, resoluble casi exclusivamente mediante el análisis de histogramas de color y luminosidad. Sin embargo, la inclusión de la clase Crepúsculo reveló la necesidad de incorporar información espacial. La segmentación por bloques y el análisis de gradientes verticales resultaron determinantes para capturar la transición de luz característica de esta franja horaria, logrando una exactitud del 88.3% en un conjunto de test independiente, lo que valida la capacidad de generalización del modelo ante imágenes no vistas.

Por otro lado, la clasificación meteorológica presentó desafíos únicos relacionados con el ruido visual (edificios, vegetación). La implementación de un algoritmo de recorte inteligente fue la clave del éxito en esta etapa, permitiendo limpiar el dataset de entrenamiento y eliminar falsos nublados. Además, el análisis de correlaciones evidenció una alta redundancia entre las variables cromáticas y de textura, lo cual justificó el uso del Análisis de Componentes Principales (PCA). Seleccionando las 15 primeras componentes, se logró reducir la dimensionalidad y evitar el sobreajuste, alcanzando un 81% de exactitud en el test final con imágenes reales propias.

Finalmente, como líneas futuras de investigación, se plantea ampliar el problema meteorológico añadiendo clases más complejas como lluvia, nieve o niebla, lo cual requeriría la exploración de nuevas características de textura. Asimismo, sería conveniente mejorar el algoritmo de segmentación de cielo mediante técnicas de detección de bordes más avanzadas para capturar contornos irregulares con mayor precisión. Por último, resultaría de gran interés comparar el rendimiento de estos modelos frente a arquitecturas de aprendizaje profundo (CNN), que, aunque suelen ofrecer una capacidad predictiva superior, su naturaleza sacrifica la interpretabilidad directa de las propiedades visuales (como el color o el contraste) que sí permite analizar nuestro enfoque de extracción manual.
