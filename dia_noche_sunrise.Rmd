...
ruta_base_imagenes <- "imagenes/train/daynight"
...
ruta_noche <- file.path(ruta_base_imagenes, "daynight/nighttime")
ruta_dia <- file.path(ruta_base_imagenes, "daynight/daytime")
ruta_amanecer <- file.path(ruta_base_imagenes, "daynight/sunrise")
...
# Combinación de los dataframes
datos_train <- rbind(df_noche, df_dia, df_amanecer)
```
...
```{r}
# Cuando se sepa seguro que variables vamos a coger al final se ponen aquí
train_x_multi <- datos_train[, -c(1,2)]#c("R_mediana", "G_mediana", "B_mediana", "H_cos_mediana", "H_sin_mediana", "S_mediana", "V_mediana", "sd_V", "Porcentaje_calidos", grep("Bloque_", colnames(datos_train), value = TRUE))]
train_y_multi <- datos_train$etiqueta

set.seed(42)
rf_dia_noche_crep <- randomForest(x = train_x_multi, y = train_y_multi, ntree = 100, importance=TRUE)
```

```{r}
save(rf_dia_noche_crep, file = "rf_dia_noche_crep.RData")
```
